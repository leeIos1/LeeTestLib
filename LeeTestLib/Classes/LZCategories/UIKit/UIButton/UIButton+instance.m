...
/** 指定title里面的字符串的字体颜色 */
+ (UIButton *)setupButtonTitle:(NSString *)title titleColor:(UIColor *)color anotherColorTitle:(NSString *)colorTitle anotherColor:(UIColor *)anotherColor titleFont:(CGFloat)fontSize withObject:(NSObject *)obj action:(SEL)action{
    if (!title) {
        title = @"";
    }
    UIButton *button = [UIButton buttonWithType:UIButtonTypeCustom];
    ...
    if ([title rangeOfString:colorTitle].location != NSNotFound) {
        ...
        [attService addAttribute:NSForegroundColorAttributeName value:anotherColor range:[title rangeOfString:colorTitle]];
