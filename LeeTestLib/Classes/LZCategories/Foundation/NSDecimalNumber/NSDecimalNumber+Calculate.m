...
//字符串是否为空
#define kLZStringIsEmpty(str) ([str isKindOfClass:[NSNull class]] || str == nil || [str length] < 1 ? YES : NO )
...
+ (NSString *)stringWithScale:(short)scale roundModel:(NSRoundingMode)roundModel originalString:(NSString *)originalString{
    if (kLZStringIsEmpty(originalString)) {
        originalString = @"0";
    }
...
- (NSString *)calculateReturnStringWithOpration:(NSString *)opration
    ...
    if (kLZStringIsEmpty(oprationString)) {
        oprationString = @"0";
    }
...
+ (NSString *)calculateReturnStringWithOpration:(NSString *)opration
                                    ...
                                    rightString:(NSString *)rightString{
    if (kLZStringIsEmpty(leftString)) {
        leftString = @"0";
    }
    if (kLZStringIsEmpty(rightString)) {
        rightString = @"0";
    }
