...
+ (NSMutableAttributedString *)changeStringFromStr:(NSString *)str colorStrArray:(NSArray *)strArray withColor:(UIColor *)color{
    //字符串是否为空
    BOOL strIsEmpty = [str isKindOfClass:[NSNull class]] || str == nil || [str length] < 1 ? YES : NO;
    //数组是否为空
    BOOL arrayIsEmpty = (strArray == nil || [strArray isKindOfClass:[NSNull class]] || strArray.count == 0) ? YES : NO;
    if (strIsEmpty || arrayIsEmpty) {
        NSMutableAttributedString * attStr = [[NSMutableAttributedString alloc]initWithString:@""];
        return attStr;
    }
...
#pragma - 因为Base64编码中包含有+,/,=这些不安全的URL字符串,所以要进行换字符加密
